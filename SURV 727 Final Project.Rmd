...
trends_high_volume <- gtrends(
  keyword = c("abortion", "abortion pill", "murder"),
  time = timeframe,
  geo = "US-FL"
...
city_df_high <- trends_high_volume$interest_by_city %>%
  filter(keyword == "abortion") #including only abortion in the high search volume df
...
```{r}
# Ensure hits column is numeric in both data frames
low_volume_interest <- low_volume_interest %>%
  mutate(hits = as.numeric(hits)) %>%
  filter(keyword != "abortion") # Exclude abortion for low-volume results

high_volume_interest <- high_volume_interest %>%
  mutate(hits = as.numeric(hits)) %>%
  filter(keyword == "abortion") # Include only abortion for high-volume results

# Combine the cleaned interest_over_time data
combined_interest_data <- bind_rows(low_volume_interest, high_volume_interest)

# Clean and filter combined data
interest_clean <- combined_interest_data %>%
  select(date, keyword, hits) %>%
  filter(!is.na(hits)) # Remove rows with NA hits
...
abortion_pill_filtered_city_df <- combined_city_data %>%
  filter(keyword == "abortion pill") #murder df with cities
...
roe_v_wade_filtered_city_df <- combined_city_data %>%
  filter(keyword == "roe v wade") #murder df with cities
```
